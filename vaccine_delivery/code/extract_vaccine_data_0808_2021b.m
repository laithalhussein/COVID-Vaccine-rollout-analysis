function [vm, va] = extract_vaccine_data_0808_2021b(path_)
close all;
home;
cd(path_);
...
disp(['We have ', num2str(length(extraneous_terr)), ' extraneous territories: ', extraneous_terr{:}]);
...
for k1=1:num_vm
    ...
    vm.(cmnft).terr(IL_idx) = vm.(cmnft).dose1(IL_idx) + vm.(cmnft).dose1(Chicago_idx);
    vm.(cmnft).terr(NY_idx) = vm.(cmnft).dose1(NY_idx) + vm.(cmnft).dose1(NYC_idx);
    vm.(cmnft).terr(PA_idx) = vm.(cmnft).dose1(PA_idx) + vm.(cmnft).dose1(Philadelphia_idx);
    
    vm.(cmnft).terr(IL_idx) = vm.(cmnft).dose2(IL_idx) + vm.(cmnft).dose2(Chicago_idx);
    vm.(cmnft).terr(NY_idx) = vm.(cmnft).dose2(NY_idx) + vm.(cmnft).dose2(NYC_idx);
    vm.(cmnft).terr(PA_idx) = vm.(cmnft).dose2(PA_idx) + vm.(cmnft).dose2(Philadelphia_idx);
    
    %remove the cities
    for k2=1:length(vm_flds)
        cvm_fld = vm_flds{k2};
        vm.(cmnft).(cvm_fld)(Chicago_idx) = [];
        vm.(cmnft).(cvm_fld)(NYC_idx) = [];
        vm.(cmnft).(cvm_fld)(Philadelphia_idx) = [];        
    end
end
