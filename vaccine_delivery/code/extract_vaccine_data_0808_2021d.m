function [vm, va] = extract_vaccine_data_0808_2021d(path_)
close all;
home;
cd(path_);
...
%% create a date ID that will permit linear indexing wrt time
...
%begin with va data
for k=1:size(va.date_num,1)
    key_idx = find(ismember(va.date_num, date_seq));;
end
